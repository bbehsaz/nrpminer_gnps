...
# "$derepdir"/misc/ad_hoc_python_scripts/graphs_to_lib_info.py "$output"_candidateNRPs.GRAPHS.txt "$output"_graphs 
...
# if [ $maxmod != 0 ]; then
# 	"$derepdir"/dereplicator.py -o "$permdir"/dereplicator --max-charge 1 --threads 4 --db-path="$output"_graphs --max-mod="$maxmod" --p-value-limit=1e-25 "$allspectfile" -l  "$output"_graphs/library.info.graphs
# else
# 	"$derepdir"/dereplicator.py -o "$permdir"/dereplicator --max-charge 1 --threads 4 --db-path="$output"_graphs --max-mod="$maxmod" --p-value-limit=1e-20 "$allspectfile" -l  "$output"_graphs/library.info.graphs
# fi
#up to here
...
if [ $maxmod != 0 ]; then
	...
	if [ ! -f "$permdir"/"$line"_all_psms.concise.tsv ]; then
		...
		wait
		python "$cyclonovoDir"/scripts/fixconsie.py "$output"_"$line"_all_psms.concise.txt 3 | sort -rnk3,3 | awk '{print $1"\t"$2"\t"$3"\t"$4}' > "$permdir"/"$line"_all_psms.concise.tsv
		wait
	fi;
