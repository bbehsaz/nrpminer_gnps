...
def index_nrps_genbank_features(
    ...
    for (index, feature) in enumerate(gb_record.features) : #go through the features in gbk 
        if feature.type==feature_type :
            if qualifier in feature.qualifiers :
                ids = [y.split()[-1] for y in [x for x in feature.qualifiers[qualifier] if 'Domain' in x] ]
                ...
                #There should only be one locus_tag per feature, but there
                #are usually several db_xref entries
                # if "Domain" in feature.qualifiers['NRPS_PKS']:
                ...
                for value in feature.qualifiers['locus_tag'] :
                    if value in answer :
                        ...
                        answer[value] = (start,end,strand,ids)
                        answer[value] = index
...
def get_final_nrpPred_per_custer(gb_record,delete_orf): 
    ...
    # ctg_orf_Anum_aSDomain_index = index_genbank_features(gb_record,"aSDomain","label") #ctg     
    ctg_nrp_index = index_nrps_genbank_features(gb_record,"CDS","NRPS_PKS")
    ...
    for cds in ctg_nrp_index: 
        ...
        for info in gb_record.features[ctg_nrp_index[cds]].qualifiers['NRPS_PKS']:
            ...
            alldomains = [y.split()[-1] for y in [x for x in gb_record.features[ctg_nrp_index[cds]].qualifiers['NRPS_PKS'] if 'Domain' in x] ]
            # if "Type" in info:
            #     product_type = info.split(":")[1][1:]
            ...
            if gb_record.features[ctg_nrp_index[cds]].strand == -1:
                allORF_NRPs_domain_names[cds] = alldomains[::-1]
    ...
    for ctg in  sorted_ctgs:  # find the groups based on conditions new_group only if: 
        ...
        all_nrp_groups_locs_ctg_pos[ctg] = new_loc
        ...
        if not new_group and distance>1000:
            new_group = True
        if new_loc[2]  == 1:
            if 'Thioesterase' in allORF_NRPs_domain_names[ctg][-1] :
            # if allORF_NRPs_domain_names[ctg][-1] == 'Thioesterase':
                finish_group = True
        elif 'Thioesterase' in allORF_NRPs_domain_names[ctg][-1] :
    ...
    return final_nrps_domains, all_nrp_groups_locs_ctg_pos
...
def create_putative_nrp_dataset(antismash_resgbk
    ,nrpspks_predictions_dir
    ,allSpecMasses
    ,cyclominerPath
    ,threshold
    ,delete_orf
    ...
    import glob
    # all_gbk_files_in_dir = glob.glob(antismash_res_dir+"/*.gbk")
    all_gbk_files_in_dir = [antismash_resgbk]
    ...
    def get_final_nrp_groups_per_ctg(per_cluster_final_nrps_domains):
        ...
        for group in per_cluster_final_nrps_domains:
            ...
            if len(PKS_domains)>1:
                continue
            all_Adomains = [domain for domain in domains if re.match(r'.*.*AMP.*', domain)]
            numAA = len(all_Adomains)
            #minimum length of assembly lines considered is 3 and maximum is 20
            if numAA<3 or numAA>20:
                continue
            total_num_groups += 1
            ctg_num = all_Adomains[0].split("_")[1]
    ...
    per_cluster_final_nrps_positions_perGBK = {}
    per_cluster_final_nrps_positions ={}
    def merge_two_libs(x, y):
        z = x.copy()
        z.update(y)
        return z
    for gb_file in all_gbk_files_in_dir:

        all_gbk_records = read_gbk_file_get_features(gb_file)
        
        for gb_record in all_gbk_records:
            # gb_record = read_gbk_file_get_features(gb_file)
            clusterName = gb_record.name
            per_cluster_final_nrps_domains, per_cluster_final_nrps_positions_perGBK = get_final_nrpPred_per_custer(gb_record,delete_orf)
            per_cluster_final_nrps_positions.update(per_cluster_final_nrps_positions_perGBK)   
            test, per_ctgs_code_File, total_num_groups = get_final_nrp_groups_per_ctg(
                per_cluster_final_nrps_domains)



    # if nrpspks_predictions_dir=='':
    #     nrpspks_predictions_dir = antismash_res_dir+"/nrpspks_predictions_txt/"


    all_nrpspredictorCode_files_in_dir = glob.glob(nrpspks_predictions_dir+"/ctg*_nrpspredictor*_codes.txt")
    all_nrpspredictorSVM_files_in_dir = glob.glob(nrpspks_predictions_dir+"/ctg*_nrpspredictor*_svm.txt")    
    all_nrpspredictorInd_files_in_dir = glob.glob(nrpspks_predictions_dir+"/ctg*_ind.res.tsv")

    all_nrpspredictorInd_files_in_dir = {}
    per_ctg_nrpspred_codes = {}
    per_ctg_nrpspred_scores = {}
    per_ctg_nrpspred_ranks = {}
    per_ctg_nrpspred_byrank = {}
    per_orf_nrpspredictor2_codes_stach ={}
    per_orf_nrpspredictor2_ ={}
    per_orf_nrpspredictor2_scores_stach = {}
    per_orf_nrpspredictor2_ranks_stach = {}
    per_orf_nrpspredictor2_AAname_stach = {}
    ...
    for code_file in all_nrpspredictorCode_files_in_dir:
        ...
        if len(per_orf_nrpspredictor2_codes_toadd):            
            per_orf_nrpspredictor2_codes_stach.update(per_orf_nrpspredictor2_codes_toadd)
            per_orf_nrpspredictor2_scores_stach.update(per_orf_nrpspredictor2_scores_toadd)
            per_orf_nrpspredictor2_ranks_stach.update(per_orf_nrpspredictor2_ranks_toadd)
            per_orf_nrpspredictor2_AAname_stach.update(per_orf_nrpspredictor2_AAname_toadd)
    ...
    for svm_file in all_nrpspredictorSVM_files_in_dir:
        per_orf_nrpspredictor2_SVMs_toadd,per_orf_nrpspredictor2_scores_SVM_toadd,per_orf_nrpspredictor2_ranks_SVM_toadd, per_orf_nrpspredictor2_AAname_SVM_toadd =  readSVMs(svm_file,topaa,minscore,cyclominerPath,cyclominerPath)
    ...
    for gbk_file in all_gbk_files_in_dir: #iterate through gbk files
        all_gbk_records = read_gbk_file_get_features(gbk_file)        
    ...
    ##################################
    # Look through the A-domains in SVM predictions 

    for orf in sorted(per_orf_nrpspredictor2_SVMs):
        addedaminos = []
        if orf in per_orf_nrpspredictor2_scores_stach:
            if not len(per_orf_nrpspredictor2_scores_stach[orf][0]) > 0:
                continue
        max_stach_score= max(per_orf_nrpspredictor2_scores_stach[orf][0])

        if orf in per_orf_nrpspredictor2_scores_stach: # going through the A-domains 
            if not len(per_orf_nrpspredictor2_scores_stach[orf][0])>0:
                continue
            ...
            for i in range(len(per_orf_nrpspredictor2_codes_stach[orf][0])):
                amino = per_orf_nrpspredictor2_codes_stach[orf][0][i]
                aminoName = per_orf_nrpspredictor2_AAname_stach[orf][0][i]
                ...
                stachcode_score = per_orf_nrpspredictor2_scores_stach[orf][0][i]
                stachcode_score_final = round((stachcode_score/(max_stach_score*1.0)),2)*100
                if amino in per_orf_nrpspredictor2_SVMs[orf]:
                    ...
                    new_score = ( stachcode_score + stachcode_score)/2.0
    ...
    ##################################
    # Look through the A-domains in Stachelhaus predictions 
    ...
    return final_nrps_info, per_cluster_final_nrps_positions
...
def output_nrpsgraph(candidateNRPSs, output,per_cluster_final_nrps_positions):
    ...
    for nrpPair in candidateNRPSs:
        compoundName = nrpPair[0].replace('nrpspksdomains_','')
        aminos= compoundName.split("_")[-1]


        ctglistseq = "_".join(compoundName.split("_")[:-3])
        ctglist = ctglistseq.split("-")
        allposlist = []
        strand = ""
        for ctgorfs in ctglist:
            # if ctgorfs in per_cluster_final_nrps_positions:
                allposlist.append(per_cluster_final_nrps_positions[ctgorfs][0])
                allposlist.append(per_cluster_final_nrps_positions[ctgorfs][1])
                if  per_cluster_final_nrps_positions[ctgorfs][2] == 1:
                    strand = "pos"
                else:
                    strand = "neg"
        ...
        minpos = min(allposlist)
        maxpos = max(allposlist)
        ctgID = ctglistseq.split("_")[0]
        nrps = nrpPair[1]
        

        nrpminerscore =  compoundName.split("_")[-3]
        compoundName = ctgID + "_" + str(minpos) + "_" + str(maxpos) + "_" + str(strand) + "_" + str(nrpminerscore) + "_" +aminos
        # compoundName = nrpPair[0]
        # compoundName  += "+" + ctgID + "_" + str(minpos) + "_" + str(maxpos) + "_" + str(strand)
        # compoundName = ctgID + "_" + str(minpos) + "_" + str(maxpos) + "_" + str(strand)
        # print compoundName
        # exit()
