...
from scripts.create_putative_nrp_database_nrpspredictor3 import output_nrpsgraph
...
def get_parser():
    ...
    parser.add_argument('--antismash_resgbk', help="antiSMASH result diroctory ", default="")
    ...
    #p-value threshold 
    parser.add_argument("--pvalue", dest="pvalue", help='Maximum mass modification to consider', default=float(1.0e-20),type=float)
...
if __name__ == "__main__":
    ...
    if len(args.antismash_resgbk) ==0 and len(args.coreNRPDB)==0:
        print "Need either an antiSMASH result directory or a core NRP database generated by NRPminer previously"
        exit()
    elif len(args.antismash_resgbk) > 0 and len(args.coreNRPDB) > 0:
    ...
    verboseprint("++ Reading antismash predictions from dir: " + args.antismash_resgbk)
    protonMass = 1.00728 #This is mass of hydrogen --- do not remove!
    digits = 3

    antiSMASH_directory_name =os.path.dirname(args.antismash_resgbk)
    ...
    if not dbpremade:
        ...
        totalnum = 0
        a = 0
        # groupname  = os.path.dirname(args.antismash_resgbk+"/").split("/")[-1]
        # gbkres = antismash_resgbk
        import os
        groupname = os.path.basename(args.antismash_resgbk)
        ...
        final_nrps_info, per_cluster_final_nrps_positions= create_putative_nrp_dataset(args.antismash_resgbk, args.nrpspks_predictions_txt, allFilteredSpecMasses,NRPminerPath,rankThreshold,args.delete_orfs,args.delete_aminos,args.topAA,args.minAdomainscore,maxMod)
        nrps2check = []
        ...
        addMass= output_nrpsgraph(nrps2check,outtemp_group,per_cluster_final_nrps_positions)
        totalnum = 0
        a = 0
    ...
    with CleanChildProcesses():
        ...
        if args.makeNRPDBonly:
            ...
            proc = Popen(["sh " +NRPminerPath+'/scripts/run_varquest_mass_nrp.sh '+outputWithAbsPath+ " " + cyclospectraAbsPath + " " + str(NRPminerPath)  + " " + str(outputDirAbsPath) + " " + str(derepDirAbsPath) + " " + str(args.e)+ " " + str(int(args.maxmod)) + " " + str(permoutdir) + " "+ groupname + " " + str(args.numthreads) + " " + str(args.pvalue)
            ] ,shell=True)
            try:
                proc.wait()
            except KeyboardInterrupt:
